...
def main_page():
    ...
    def calculate_hp():
        """Berechnet HP nach PHB-Regel: Full Die (lvl 1) + (Die/2 + 1 + Con) * Rest-Lvl"""
        hit_die = 10 # Standard für Ranger/Fighter
        con_mod = get_mod_value(hero.stats["Konstitution"])
        if hero.level <= 1:
            return hit_die + con_mod
        else:
            # PHB: 1. Level voll, danach Schnitt (6 bei d10) + Con
            return (hit_die + con_mod) + (hero.level - 1) * (6 + con_mod)

    async def handle_upload(e: events.UploadEventArguments):
        try:
            # 1. DATEINAME ALS NAMENS-QUELLE (Dein Vorschlag)
            # Entfernt Endungen wie .pdf und Zahlenfolgen am Ende
            raw_name = e.name.split('.')[0] # 'Faenor Thia_3142026'
            clean_name = re.sub(r'_\d+$', '', raw_name).replace('_', ' ')
            hero.name = clean_name
            ...
            # 2. KLASSE & LEVEL (PrismScroll Erkennung)
            if "ranger" in full_text.lower(): hero.class_name = "Revised Ranger"
            lvl_match = re.search(r'(?:level|lvl)\s*(\d+)', full_text, re.IGNORECASE)
            hero.level = int(lvl_match.group(1)) if lvl_match else 1
            ...
            # 4. AC & HP BERECHNUNG
            ac_match = re.search(r'(?:ac|armor class)\s*(\d+)', full_text.lower())
            hero.ac = int(ac_match.group(1)) if ac_match else (10 + get_mod_value(hero.stats["Geschicklichkeit"]))
            
            hero.hp_max = calculate_hp()
    ...
    with ui.column().classes('w-full items-center q-pa-md max-w-2xl mx-auto'):
        ui.label('🐺 Wolperting v3.8').classes('text-h3 text-primary font-bold q-mb-md')
        ...
        with ui.card().classes('w-full q-pa-md shadow-lg border-t-8 border-primary'):
            ...
            name_input = ui.input('Held', value=hero.name).classes('text-h5 w-full q-mt-md')
        ...
        with ui.row().classes('w-full q-mt-md gap-4 justify-center'):
            with ui.card().classes('col q-pa-md items-center shadow-md bg-red-50 border-red-200'):
                ui.label('HP (PHB-Lvl)').classes('text-xs font-bold text-red-600')
        ...
        ui.button('ÄNDERUNGEN SPEICHERN', on_click=lambda: save_hero(hero)).classes('w-full q-mt-xl py-4 text-xl shadow-xl')
...
def save_hero(char):
    ...
    ui.notify('Daten gesichert!', color='positive')
